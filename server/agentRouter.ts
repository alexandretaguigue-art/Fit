...
export const agentRouter = router({
  ...
  generateNutritionPlan: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const prompt = [
        "Expert nutrition sport. Reponds en JSON UNIQUEMENT, sans markdown.",
        "",
        ...
        "REGIME: " + (input.diet || "aucun") + " | eviter: " + (input.avoid || "/") + " | preferes: " + (input.foodPrefs || "/"),
        "REPAS/JOUR: " + input.mealsPerDay + " avec ids=" + mealIds.join(","),
        "",
        "Calcul kcal: Mifflin-St Jeor + facteur(sedentaire=1.2, leger=1.375, modere=1.55, actif=1.725)",
        "+ delta(lean_bulk=+250, cut=-400, recomp=0, perf=+200, sinon=0).",
        "Proteines: muscu=1.8g/kg, endurance=1.4g/kg, sinon=1.6g/kg.",
        "Genere exactement 7 jours (Lundi a Dimanche). MAX 3 aliments par repas. note=null sauf si utile.",
        "",
        'FORMAT EXACT: {"targets":{"kcal":0,"pro":0,"glu":0,"lip":0},"rationale":"1 phrase","week":[{"dayName":"Lundi","meals":[{"id":"b","icon":"emoji","name":"Petit-dej","time":"7h","kcal":0,"pro":0,"glu":0,"lip":0,"hypo":false,"note":null,"foods":[{"n":"Flocons avoine","g":80,"kcal":300,"p":10,"gl":54,"l":5}]}]}]}',
      ].join("\n");
  ...
  generateSportPlan: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const prompt = [
        "Coach sportif expert. Reponds en JSON UNIQUEMENT, sans markdown.",
        "",
        ...
        "",
        "Genere 1 semaine avec " + input.sessionsPerWeek + " seances + repos. MAX 5 exercices/seance.",
        "Charges realistes selon niveau. Progression muscu: +2.5kg si toutes reps OK.",
        "",
        'FORMAT EXACT: {"program_name":"Mon Programme","goal_statement":"1 phrase","weeks":[{"week_number":1,"theme":"Semaine 1","days":[{"day":"Lundi","type":"strength","name":"Push Day","duration_min":60,"exercises":[{"id":"dc","name":"Developpe couche","sets":3,"reps":"10","weight_kg":60,"rest_sec":90,"progression":"+2.5kg si OK","notes":"Dos plat"}]}]}]}',
      ].join("\n");
  ...
  chatNutrition: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const system = [
        "Tu es un agent nutritionniste expert. Reponds en francais, concis et pratique.",
        ...
        "REFERENCES: pizza=700-800kcal, burger+frites=900-1100kcal, pates resto=700-1000kcal.",
        "Ne jamais sous-estimer les quantites. Reste positif.",
      ].join("\n");
  ...
  chatSport: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const system = [
        ...
        "PRINCIPES: surcharge progressive (+2.5kg si toutes reps OK), max +10% volume/semaine.",
      ].join("\n");
